...
- 🔇 **ノイズ除去**: FFT/NLMeansベース（afftdn/anlmdn）でホワイトノイズを効果的に除去
...
#### 音声処理
- **ノイズ除去**: ON/OFF切り替え + ノイズフロア閾値調整（-60～-30dB、推奨: -50dB）
  - FFTベース（afftdn）またはNLMeansベース（anlmdn）を自動選択
  - ホワイトノイズ、定常ノイズに効果的
...
Stage 2: Denoise（ノイズ除去）
  ...
  ↓ FFT Denoise（afftdn）またはNLMeans（anlmdn）
  ↓ ホワイトノイズ・定常ノイズを除去
...
- 「ノイズフロア閾値」を下げる（-55dB～-60dB等）
- FFT/NLMeansフィルタが自動適用され、ホワイトノイズに効果的
...
| **ノイズ除去** | FFT/NLMeans（afftdn/anlmdn） | スペクトラルゲーティング（noisereduce） |
| **コスト** | 完全無料 | 無料（ローカル処理） |
...
- **Week 3**: 詳細設定UI、設定共有URL、FFT/NLMeansノイズ除去、ドキュメント
